class Question < ActiveRecord::Base
  ...
  scope :votes_count, -> { select('questions.*, sum(votes.value) as votes_count').joins('LEFT OUTER JOIN votes ON questions.id = votes.votable_id').group('questions.id').order('votes_count DESC') }
